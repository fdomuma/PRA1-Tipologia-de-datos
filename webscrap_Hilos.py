...
try:
    ...
    print("Fin pagina")
    time.sleep(10) #necesitamos 2000 segundos hasta que la pagina llega al final
    print("Fin time")
except:
    print("error") 
...
inicio = 1
t0 = time.time()
# funcion para probar la ejecucion de cada hilo
def testThread(link):
...
#numero de hilos
MAX_THREADS = 30
threads = min(MAX_THREADS, numMax)
#configuramos el future
executor = concurrent.futures.ThreadPoolExecutor(max_workers=threads)
#ejecutamos la funcion para cada link
results = executor.map(testThread, enlacesObras)

for value in results:
    datos.append(value)

t1 = time.time()
print(f"{t1-t0} segundos para descargar todos los links.")
